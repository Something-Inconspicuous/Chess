...
public boolean validMove(int r, int c) {
		...
		//make liek a function interface that applies a function on a certain loop if I were to ooptimize this in the future
		
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				if (!tempMap.containsKey((char) (65 + kingCol + 2 * (2 * i - 1)) + "" + (8 - (kingRank + (2 * j - 1))))) {
					continue;
				}

				if (board[kingRank + (2 * j - 1)][kingCol + 2 * (2 * i - 1)] != null
						&& board[kingRank + (2 * j - 1)][kingCol + 2 * (2 * i - 1)].getColor() != getColor() && board[kingRank + (2 * j - 1)][kingCol + 2 * (2 * i - 1)].getNameChar() == 'N') {
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				if (!tempMap.containsKey((char) (65 + kingCol + (2 * j - 1)) + "" + (8 - (kingRank + 2 * (2 * i - 1))))) {
					continue;
				}

				if (board[kingRank + 2 * (2 * i - 1)][kingCol + (2 * j - 1)] != null
						&& board[kingRank + 2 * (2 * i - 1)][kingCol + (2 * j - 1)].getColor() != getColor() && board[kingRank + 2 * (2 * i - 1)][kingCol + (2 * j - 1)].getNameChar() == 'N') {
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 1; i <= Math.abs(j * 7 - kingCol); i++) {
				...
				if (board[kingRank][(2 * j - 1) * i + kingCol] != null) {
					if (board[kingRank][(2 * j - 1) * i + kingCol].getColor() != getColor() && board[kingRank][(2 * j - 1) * i + kingCol].getValue() >= 5 ) {
						board[r][c] = prevPiece;
						board[prevRank][prevCol] = this;
		...
		if (kingCol != 0 && board[kingRank + ((isWhite) ? -1 : 1)][kingCol - 1] != null
				&& board[kingRank + ((isWhite) ? -1 : 1)][kingCol - 1].getColor() != getColor() &&  board[kingRank + ((isWhite) ? -1 : 1)][kingCol - 1].getNameChar() == 'P') {
			board[r][c] = prevPiece;
			board[prevRank][prevCol] = this;
